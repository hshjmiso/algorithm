...
function averageWaitingTime(customers: number[][]): number {
    let answer:number = 0;
    let preparingTime:number = 0;

    customers.forEach(([arrival, time]) => {
        preparingTime = arrival > preparingTime ? arrival + time : preparingTime + time;
        answer += preparingTime - arrival;
    });

    return answer / customers.length;
};

// Minimum Runtime, Minimum Memory
function averageWaitingTime_rm(customers: number[][]): number {
